...
class TrajectoryTracker:
    ...
    def __init__(self, scale_factor=0.001):
        ...
        # Trajectory display range limits
        ...
        self.vx_sign = +1   # Car body X direction: forward is positive (front direction)
        self.vy_sign = +1   # Car body Y direction: left is positive (left side from front view)
    ...
    def update_from_frame(self, frame_info):
        """Update trajectory from CAN frame info (car body frame to world frame conversion)"""
        ...
        if has_vx and has_vy and has_vtheta and dt > 0:
            # Velocity filtering
            vx_body = self._filter_velocity(self.vx_cache, self.vx_history)  # mm/s (car body frame)
            vy_body = self._filter_velocity(self.vy_cache, self.vy_history)  # mm/s (car body frame)
            vtheta_body = self._filter_velocity(self.vtheta_cache, self.vtheta_history)  # mrad/s (milliradians/second)
            ...
            # Convert car body frame velocity to world frame velocity
            ...
            # Update pose
            self.pose.x += vx_world * dt  # meters (world frame)
            self.pose.y += vy_world * dt  # meters (world frame)
    ...
    def get_trajectory_arrays(self):
        ...
        # Downsample and convert outside lock
    ...
    def get_current_pose(self):
        """Get current pose (world frame)"""
        return self.pose
    
    def get_current_velocity(self):
        """Get current velocity (vx_m/s, vy_m/s, vtheta_rad/s) - world frame"""
        # Get latest car body frame velocity
    ...
    def get_trajectory_length(self):
        ...
        # Use numpy vectorized calculation for performance (execute outside lock)
