% M sweep for the proposed TII detector and four baselines:
...
addpath(script_dir);
data_output_file=fullfile(release_root,'data','FDR_TII_mainR1_M_compare.mat');
...
spow=1;
dd=1/2;
upm_delta=0.15;          % DCISA power reduction strength
...
FDR_results=zeros(num_scenarios,num_methods,num_m);
PERR_results=zeros(num_scenarios,num_methods,num_m);
PFA_test_results=zeros(num_scenarios,num_methods,num_m);
PMD_test_results=zeros(num_scenarios,num_methods,num_m);
Gate_TII=zeros(N_h,num_m);
Gate_ERD=zeros(N_h,num_m);
Gate_Spatial=zeros(N_h,num_m);
Gate_Random=zeros(N_h,num_m);
PFA_train_TII=zeros(N_h,num_m);
PFA_train_ERD=zeros(N_h,num_m);
PFA_train_Spatial=zeros(N_h,num_m);
...
%% Common random objects for paired M sweep
...
for im=1:num_m
    ...
    for h=1:N_h
        ...
        attack_scale=powerMatchAttackScale(h0_bank(:,h),g_nominal,upm_gain);
        if attack_scale<=0
            attack_scale=combo_attack_boost;
        end
        g_combo=attack_scale*g_nominal;
    ...
    for h=1:N_h
        ...
        Gate_TII(h,im)=Gate_tii;
        Gate_ERD(h,im)=Gate_erd;
        Gate_Spatial(h,im)=Gate_spatial;
        Gate_Random(h,im)=Gate_random;
        PFA_train_TII(h,im)=mean(D_tii_na>Gate_tii);
        PFA_train_ERD(h,im)=mean(abs(erd_stat_na-erd_center)>Gate_erd);
        PFA_train_Spatial(h,im)=mean(spatial_sim_na<Gate_spatial);
    ...
    FDR_results(:,:,im)=mean(FDR_h,3);
    PFA_test_results(:,:,im)=mean(PFA_test_h,3);
    PMD_test_results(:,:,im)=mean(PMD_test_h,3);
    PERR_results(:,:,im)=FDR_results(:,:,im);
end
...
m_labels=compose('M_%d',M_list);

for scenario_idx=1:num_scenarios
    T=array2table(squeeze(FDR_results(scenario_idx,:,:)), ...
        'VariableNames',m_labels,'RowNames',method_labels);
...
save(data_output_file, ...
    ...
    'M_list','snr_db','Numx','spow','dd','upm_delta','upm_gain_target','upm_spread','pe','combo_attack_boost','sense_error','theta','theta_p', ...
...
% A positive gap favors rank two; its no-attack upper quantile supplies a
% finite-sample confidence gate without changing the MDL model itself.
tau_p=size(Y,2);
