...
class PathCorrectionController:
    ...
    When trajectory deviation from the ideal path is detected, automatically adjust motion control to keep the vehicle close to the ideal trajectory
    """
    ...
    def __init__(self, trajectory_tracker=None):
        ...
        self.standalone_base_speed = 100  # Standalone correction mode base speed (mm/s)
        ...
        self.target_arrival_threshold = 0.05  # Target arrival threshold (meters)
        ...
        self.correction_start_time = 0  # Correction planned start time
        self.correction_delayed = False  # Whether in delay waiting state
        
        # New: correction execution status
        ...
        self.correction_execution_start_time = 0  # Correction actual execution start time
        ...
        self.points_per_command = 5  # Points generated per command (for fixed mode)
    ...
    def body_to_world(self, vx_body: float, vy_body: float, theta_deg: float) -> Tuple[float, float]:
        """
        Vehicle coordinate system velocity -> World coordinate system velocity
        
        Args:
            vx_body: forward speed (mm/s)
            vy_body: leftward speed (mm/s)
            theta_deg: heading angle (degrees), counter-clockwise is positive
            
        Returns:
            (vx_world, vy_world): world coordinate system velocity (mm/s)
        """
    ...
    def world_to_body(self, vx_world: float, vy_world: float, theta_deg: float) -> Tuple[float, float]:
        """
        World coordinate system velocity -> Vehicle coordinate system velocity
        ...
        Returns:
            (vx_body, vy_body): vehicle coordinate system velocity (mm/s)
        """
        theta_rad = math.radians(theta_deg)
    ...
    def set_target_path(self, path_points: List[Point]):
        ...
        Args:
            path_points: target path point list (world coordinate system)
        """
        ...
        # Record path information to log
        self.path_log = []
        self.log_path_info(f"=== Set target path ===")
        self.log_path_info(f"Path point count: {len(path_points)}")
    ...
    def set_ideal_trajectory_from_queue(self, speed_queue, density_mode="auto", points_per_command=None):
        ...
        Args:
            ...
            points_per_command: points generated per command (only for fixed mode)
        """
        if not speed_queue:
            return
        ...
        # Generate trajectory according to density mode
        ...
        if len(ideal_points) >= 2:
            ...
            self.log_path_info(f"Path total length: {total_length:.3f}m")
    ...
    def _generate_path_simple(self, speed_queue, density_mode=None):
        """Generate trajectory points according to density mode"""
        ...
        # Set time step and sampling interval according to density mode
        if density_mode == "auto":
            ...
            base_dt = 0.01  # 10ms time step, more fine-grained
            ...
            min_distance = 0.02  # 2cm minimum interval, adaptive according to speed
        else:
            base_dt = 0.02
            sample_interval = 5
            min_distance = 0.01
        ...
        for command_idx, command in enumerate(speed_queue):
            ...
            # Adaptive mode: adjust sampling interval according to speed
            ...
            for step in range(num_steps):
                ...
                # Vehicle velocity -> World velocity
                vx_world = vx_body * math.cos(theta_rad) - vy_body * math.sin(theta_rad)
                vy_world = vx_body * math.sin(theta_rad) + vy_body * math.cos(theta_rad)
                ...
                # Record points according to density mode
                record_point = False
                ...
                if record_point:
                    point = Point(x, y)
                    # Check minimum distance (manual mode strict, other modes relaxed)
                    if density_mode == "manual":
                        if point.distance_to(points[-1]) >= min_distance:
                            points.append(point)
                    elif density_mode == "adaptive":
                        # adaptive mode: adjust minimum distance according to speed
                        if point.distance_to(points[-1]) >= min_distance:
                            points.append(point)
                    else:
    ...
    def enable_correction_with_delay(self, delay_seconds: float = None):
        ...
        """
        if not self.target_path:
            raise ValueError("Please set a target path first")
        ...
        if self.correction_delay > 0:
            ...
            self.correction_enabled = False  # Do not enable correction during delay period
            self.correction_executing = False
            
            self.log_path_info(f"Correction delay started: will start after {self.correction_delay:.1f} seconds")
    ...
    def mark_queue_finished(self):
        """Mark queue execution complete, but correction continues"""
        self.queue_finished = True
        self.log_path_info("=== Queue execution complete ===")
        self.log_path_info("Queue commands finished sending, starting standalone correction mode")
        self.log_path_info("Correction will continue until target reached or timeout")
    ...
    def check_and_start_correction(self):
        ...
        if current_time >= self.correction_start_time:
            ...
            if int(remaining_time) % 5 == 0 and int(remaining_time) != int(self.debug_info.get('last_remaining_time', -1)):
                self.log_path_info(f"Correction delaying, remaining: {remaining_time:.1f}s")
    ...
    def find_nearest_point_on_path(self, current_pos: Point) -> Tuple[Point, int, float]:
        ...
        for i, segment in enumerate(self.target_path_segments):
            # Calculate point-to-line-segment projection
    ...
    def should_continue_correction(self):
        """
        Determine whether correction should continue (improved version)
        ...
        if not self.correction_enabled:
            self.log_path_info("Correction not enabled, stopping")
            return False, "Correction not enabled"
        
        if not self.correction_executing:
            self.log_path_info("Correction not executed, stopping")
            return False, "Correction not executed"
        
        # Check if path is already complete
        if self.path_completed:
            self.log_path_info("Path completed (flagged), stopping")
        ...
        if self.target_position and hasattr(self, 'debug_info') and 'current_progress' in self.debug_info:
            ...
            self.log_path_info(f"Correction status check: running {elapsed_time:.1f}s, progress {progress:.1f}%, remaining {self.max_correction_time - elapsed_time:.1f}s")
        
        if elapsed_time > self.max_correction_time:
            self.path_completed = True
            self.stop_reason = f"Correction timeout ({self.max_correction_time}s)"
            self.stop_details = f"Execution time: {elapsed_time:.1f}s exceeded maximum limit {self.max_correction_time}s"
    ...
    def calculate_correction_commands(self, current_pose):
        """
        Calculate correction control commands (critical fix version - supports arbitrary path directions)
        ...
        # Check if correction should stop
        should_continue, stop_reason = self.should_continue_correction()
        if not should_continue:
            if stop_reason:
                self.log_path_info(f"Correction stop reason: {stop_reason}")
        ...
        if self.target_position:
            ...
            # Determine if destination is reached
        ...
        # ===== Critical fix: calculate velocity based on path direction =====
        if self.queue_finished and self.target_path_segments:
            ...
            if segment_idx < len(self.target_path_segments):
                ...
                if path_length_seg > 0.001:
                    ...
                    # Forward speed magnitude (decelerate according to distance)
                    ...
                    # lateral_error > 0 means on the left side of path, need to correct right (positive direction)
                    ...
                    # Combined velocity in world coordinate system
            ...
            # Queue mode: only lateral correction (world coordinate system Y direction)
            # Note: here assumes path roughly along X-axis, for precise control use path direction calculation above
            vx_world = 0
        ...
        # Convert world coordinate system velocity to vehicle coordinate system
        ...
        # Record correction information (for debugging)
        elapsed_time = current_time - self.correction_execution_start_time
        progress = self.debug_info.get('current_progress', 0.0) * 100
        
        mode = "Standalone correction" if self.queue_finished else "Queue correction"
        self.log_path_info(f"{mode}: position({current_pos.x:.3f}, {current_pos.y:.3f})")
        self.log_path_info(f"  Error: lateral {lateral_error:.3f}m, angular {angular_error:.1f}°")
        ...
        self.log_path_info(f"  Vehicle velocity: X={vx_body:.1f}mm/s, Y={vy_body:.1f}mm/s, angular={angular_speed_correction:.1f}mrad/s")
        self.log_path_info(f"  Correction strength: {self.correction_strength:.1f}")
    ...
    # New method: get correction stop reason
    def get_correction_stop_reason(self):
        """
        Get detailed reason for correction stop
        
        Returns:
            str: stop reason, returns "Running" if still running
        """
        if not self.correction_enabled:
        ...
        if self.correction_delayed:
            ...
            return f"Correction delaying, {remaining:.1f}s remaining"
        
        if not self.correction_executing:
            return "Correction not executed"
    ...
    # New method: display stop reason in terminal
    ...
    # New method: display correction completion info in terminal
    def print_correction_completion(self, reason, position_error, angle_error, elapsed_time):
        """Print correction completion info in terminal"""
        print("=" * 50)
    ...
    # New method: display correction status in terminal
    def print_correction_status(self):
        ...
        status_lines = [
            ...
            f"Target path point count: {len(self.target_path)}",
    ...
    # New method: display detailed status report
    ...
    def start_standalone_correction_loop(self, gui_instance, callback_interval_ms=100):
        ...
        Args:
            gui_instance: GUI instance, used to send commands
        ...
        self.log_path_info("=== Starting standalone correction loop ===")
    ...
    def _standalone_correction_loop(self, gui_instance, callback_interval_ms):
        ...
        # Check if correction should continue
        should_continue, stop_reason = self.should_continue_correction()
        ...
        try:
            ...
            if current_pose:
                ...
                if corrected_commands:
                    ...
                    if success:
                        self.log_path_info(f"Sending standalone correction command: X={corrected_commands['x_speed']}, Y={corrected_commands['y_speed']}, ang={corrected_commands['angular_speed']}")
        ...
        # Schedule next execution - always schedule next execution
        try:
            ...
            self.log_path_info(f"Failed to schedule next execution: {str(e)}")
            # Try to restart loop
            try:
    ...
    def get_detailed_points_info(self) -> Optional[Dict[str, Any]]:
        ...
        for i, point in enumerate(self.target_path):
            ...
            detailed_info['points'].append({
                ...
                'is_key_point': i % 10 == 0 or i == len(self.target_path)-1  # Mark every 10th point as key point
            })
